...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($parent_id = 0) {
		if(isset($this->request->get['filter_name'])){
			...
			$category_data = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) AND UPPER(cd.name)  LIKE UPPER('%".$this->db->escape(strtoupper($this->request->get['filter_name']))."%') ORDER BY c.sort_order, cd.name ASC");

			$category_data = $category_data->rows;
			...
			if (!$category_data) {
				...
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) WHERE cd.name = '" . (int)$parent_id . "'  ORDER BY c.sort_order, cd.name ASC");
	...
	public function getCategoriesById($parent_id = 0) {
		

			$category_data = $this->db->query("SELECT * FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int)$parent_id . "'");
