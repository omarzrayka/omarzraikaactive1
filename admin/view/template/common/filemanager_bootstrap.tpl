<div class="modal-dialog modal-lg">
	  ...
	  <div class="modal-body">
		...
		<div class="row">
		  ...
		  <div class="col-sm-2 text-center">
			...
			<div class="text-center"><a href="<?php echo $image['href']; ?>" class="directory" style="vertical-align: middle;"><i class="fa fa-folder fa-4x"></i></a></div>
			<label>
			  <input type="checkbox" name="path[]" value="<?php echo $image['path']; ?>" />
			  <?php echo $image['name']; ?></label>
			<?php } ?>
			<?php if ($image['type'] == 'image') { ?>
			<a href="<?php echo $image['href']; ?>" class="thumbnail"><img src="<?php echo $image['thumb']; ?>" alt="<?php echo $image['name']; ?>" title="<?php echo $image['name']; ?>" style="max-width: 64px !important;max-height: 64px !important;"/></a>
			<label>
			  <input type="checkbox" name="path[]" value="<?php echo $image['path']; ?>" />
			  <?php echo $image['name']; ?></label>
			<?php } ?>
  ...
  $('a.thumbnail').on('click', function(e) {
	  ...
	  $('#<?php echo $target; ?>').attr('value', $(this).parent().find('input').attr('value')).change();
  ...
  $('a.directory').on('click', function(e) {
	  ...
	  currenct_dir = getUrlVars($(this).attr('href'))["directory"]
  	  file_manager_page = 1;
	  $('#modal-image').load($(this).attr('href'));
  });
  
  $('.modal-footer .links a').on('click', function(e) {
	  e.preventDefault();
  
  		
	  if("directory" in getUrlVars($(this).attr('href'))){
		   currenct_dir = getUrlVars($(this).attr('href'))["directory"]
		}
		
	  if("page" in getUrlVars($(this).attr('href'))){
		 file_manager_page =  getUrlVars($(this).attr('href'))["page"]
	  }
	  $('#modal-image').load($(this).attr('href'));
  });
  ...
  $('#button-search').on('click', function(e) {
	  var url = 'index.php?route=common/filemanager_bootstrap&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>';
  ...
  $('#button-upload').on('click', function() {
	  ...
	  timer = setInterval(function() {
		  if ($('#form-upload input[name=\'file\']').val() != '') {
			  ...
			  $.ajax({
				  url: 'index.php?route=common/filemanager_bootstrap/upload&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>',
				  type: 'post',
				  dataType: 'json',
 ...
 /* $('#button-folder').on('shown.bs.popover', function() {
	  $('#button-create').on('click', function() {
		  $.ajax({
			  url: 'index.php?route=common/filemanager_bootstrap/folder&token=<?php echo $token; ?>&directory=<?php echo $directory; ?>',
			  type: 'post',
			  dataType: 'json',
  ...
  $('#modal-image #button-delete').on('click', function(e) {
	  if (confirm('<?php echo $text_confirm; ?>')) {
		  $.ajax({
			  url: 'index.php?route=common/filemanager_bootstrap/delete&token=<?php echo $token; ?>',
			  type: 'post',
			  dataType: 'json',
